...
allowed-tools: Read, Write, Edit, Bash(npm:*), Bash(npx:*), Bash(xcodebuild:*), Bash(swift:*)
---
...
**For iOS (Swift/TCA):**
```swift
func testIssue$ARGUMENTSFix() async {
    // Arrange - set up the scenario that caused the bug
    // Act - execute the code that previously failed
    // Assert - verify it now works correctly
}
...
# Build
xcodebuild -scheme RIZQ -destination 'platform=iOS Simulator,name=iPhone 15' build

# Tests
xcodebuild -scheme RIZQ -destination 'platform=iOS Simulator,name=iPhone 15' test
```
